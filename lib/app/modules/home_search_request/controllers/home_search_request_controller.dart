import 'package:cite_finder_admin/app/data/models/home_search_request.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:fluttertoast/fluttertoast.dart';
import 'package:get/get.dart';

import '../../../utils/config.dart';
import '../../../utils/new_utils.dart';

class HomeSearchRequestController extends GetxController {
  HomeSearchRequestController() : super();

  static HomeSearchRequestController get to => Get.find();

  Stream<List<HomeSearchRequest>> searchRequestStream() {
    final collection = FirebaseFirestore.instance
        .collection(Config.firebaseKeys.homeSearchRequest);

    final stream = queryCollection(collection, queryBuilder: (requests) {
      return requests.orderBy("dateAdded", descending: true);
    }, objectBuilder: (map) {
      return HomeSearchRequest.fromMap(map);
    });

    return stream;
  }

  Future<void> updateRequest(
      DocumentReference ref, Map<String, dynamic> updateMap) async {
    try {
      await ref.update(updateMap);

      Fluttertoast.showToast(msg: "Request updated successfully!");
    } catch (e) {
      Fluttertoast.showToast(msg: "An unexpected error occured");
    }
  }

  Future<void> deleteRequest(DocumentReference ref) async {
    runAsyncFunction(() async {
      try {
        await ref.delete();

        Fluttertoast.showToast(msg: "Request deleted successfully!");
      } catch (e) {
        Fluttertoast.showToast(msg: "An unexpected error occured");
      }
    });
  }
}
